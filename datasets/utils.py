...
def wrap(img, pad_size):
	h, w, _ = img.shape
	img = wrap_boundary_rgb(img, (h+pad_size*2, w+pad_size*2))
	img = np.hstack((img[:,-pad_size:,:],img[:,:w+pad_size,:]))
	img = np.vstack((img[-pad_size:,:,:],img[:h+pad_size,:,:]))
	return img.astype(np.float32)

def wrap_boundary_rgb(img, img_size):
