...
class RRG2(nn.Module):
    def __init__(self, n_feat, n_MRB, height, width, chan_factor, bias=False, groups=1):
        super().__init__()
        self.conv_in = nn.Conv2d(n_feat, n_feat, kernel_size=3, stride=1, padding=1, bias=bias)
        modules_body = [MRB(n_feat, height, width, chan_factor, bias, groups) for _ in range(n_MRB)]
        modules_body.append(nn.Conv2d(n_feat, n_feat, kernel_size=3, stride=1, padding=1, bias=bias))
        self.body = nn.Sequential(*modules_body)
...
class RRGModule(nn.Module):
    def __init__(self, in_dim, out_dim, dim, n_MRB=2, height=3, width=2, chan_factor=1.5, bias=False, groups=1):
        super().__init__()
        self.out_dim = out_dim
        self.conv_in = nn.Conv2d(in_dim, dim, kernel_size=3, stride=1, padding=1, bias=bias)
        modules_body = [MRB(dim, height, width, chan_factor, bias, groups) for _ in range(n_MRB)]
        modules_body.append(nn.Conv2d(dim, out_dim, kernel_size=3, stride=1, padding=1, bias=bias))
        self.body = nn.Sequential(*modules_body)
...
class MIRNet_v2(nn.Module):
    def __init__(self,
        ...
        n_feat = dim
        self.conv_in = nn.Conv2d(in_dim, n_feat, kernel_size=3, padding=1, bias=bias)
        ...
        for i in range(n_RRG):
            ...
            # else:
            #     groups = 4
            # modules_body.append(RRG(n_feat, n_MRB, height, width, chan_factor, bias, groups=groups))
            modules_body.append(RRG(n_feat, n_MRB, height, width, chan_factor, bias, groups=1))

        self.body = nn.Sequential(*modules_body)
        self.conv_out = nn.Conv2d(n_feat, out_dim, kernel_size=3, padding=1, bias=bias)
...
class MIRNet_v2m(nn.Module):
    def __init__(self,
        ...
        for i in range(n_RRG):
            ...
            #     groups = 4
            # modules_body.append(RRG(n_feat, n_MRB, height, width, chan_factor, bias, groups=groups))
            modules_body.append(RRG2(n_feat, n_MRB, height, width, chan_factor, bias, groups=1))
